...
const ACCESS_OPTIONS = ["public", "password", "hidden"];
...
export default function Galleries() {
  ...
  const filteredGalleries = useMemo(() => {
    ...
    return galleries.filter((gallery) => {
      ...
      const matchesAccess =
        accessFilter === "all" || accessMode === accessFilter;

      return matchesSearch && matchesStatus && matchesAccess;
    });
  }, [accessFilter, galleries, searchQuery, statusFilter]);
  ...
  async function toggleGalleryStatus(gallery) {
    ...
    setGalleries((current) =>
      current.map((item) => (item.id === gallery.id ? data : item)),
    );
  ...
  async function removeGallery(gallery) {
    const ok = window.confirm(
      `Delete "${gallery.title || "Untitled Gallery"}" from client galleries?`,
    );
    if (!ok) return;
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg }}>
      ...
      <main style={{ padding: "2.5rem 2rem" }}>
        ...
        >
          <div>
            ...
            >
              Create Pixieset-style client collections with gallery-specific
              uploads, photo sets, design controls, and publish status.
            </p>
          </div>
        ...
        {!loading && galleries.length === 0 && (
          ...
          >
            No client galleries yet. Create one to start building a collection
            workspace.
          </div>
        )}
        ...
        {!loading && galleries.length > 0 && (
          <>
            <section
              className="gallery-management-toolbar"
              aria-label="Search and filter client galleries"
            >
              <div className="gallery-management-controls">
                <label className="gallery-management-search">
                  <FieldLabel>Search Galleries</FieldLabel>
                  <input
                    type="search"
                    value={searchQuery}
                    onChange={(event) => setSearchQuery(event.target.value)}
                    placeholder="Search title, client, email, or gallery link"
                    style={inputStyle}
                  />
                </label>
              ...
              <div className="gallery-management-results">
                <span>
                  Showing {filteredGalleries.length} of {galleries.length}{" "}
            ...
            {filteredGalleries.length === 0 ? (
              <div className="gallery-filter-empty">
                <strong>No galleries match your current search or filters.</strong>
                <span>Try another name, status, or access type.</span>
                <button
                  type="button"
                  onClick={clearFilters}
                  style={secondaryButton}
                    ...
                    >
                      ...
                      >
                        <div style={{ overflowWrap: "anywhere" }}>
                          {gallery.client_name || "No client name"}
                        </div>
                        <div style={{ overflowWrap: "anywhere" }}>
                          {gallery.client_email || "No client email"}
                        </div>
                        <div
                          style={{
                            marginTop: 12,
      ...
      <style>{`
        ...
        .gallery-management-search {
          min-width: 0;
        }

        .gallery-management-results {
          align-items: center;
        ...
        .gallery-filter-empty strong {
          color: ${COLORS.white};
          font-size: 0.95rem;
        }

        .gallery-filter-empty span {
          font-size: 0.82rem;
        }
        ...
        @media (max-width: 560px) {
          ...
          .gallery-management-results > button {
            width: 100%;
          }
        }
      `}</style>
